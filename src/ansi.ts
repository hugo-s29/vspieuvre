                ...
                for (let j = 0; j < codes.length; j++) {
                    ...
                    if (code === 0) {
                        ...
                        currentStyles = currentStyles.filter(
                            (s) => !s.includes('font-weight'),
                        );
                    } else if (code === 23) {
                        // Not italic
                        currentStyles = currentStyles.filter(
                            (s) => !s.includes('font-style'),
                        );
                    } else if (code === 24) {
                        // Not underline
                        currentStyles = currentStyles.filter(
                            (s) => !s.includes('text-decoration'),
                        );
                    } else if (code === 27) {
                        // Not inverse
                        [currentFg, currentBg] = [currentBg, currentFg];
...
export function createThemeAwareAnsiConverter(): AnsiToHtml {
    ...
    const colors: any = vscode.workspace
    ...
    return new AnsiToHtml({
        fg: theme.kind === vscode.ColorThemeKind.Dark ? '#FFFFFF' : '#000000',
        bg: theme.kind === vscode.ColorThemeKind.Dark ? '#0000' : '#0000',
        colors: ansiColors,
        newline: true,
        escapeXML: true,
    });
